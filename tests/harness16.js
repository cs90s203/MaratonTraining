...
(async () => {
  ...
  const pushes = []; Store._cloudPushPlanOverride = (wn, doc) => pushes.push(wn);
  ...
  assert(J(now.duration) === J({ min: 45, max: 50 }) && pushes.length === n0 + 1 && now.derived === false, 'duration edit saved to the shared week');
  html = card(W, D, run.id);
