...
(async () => {
  ...
  assert(!page.includes('class="drag-handle"') && !page.includes('本週課表設定') && !page.includes('A.toggleItemPicker(') && !page.includes('A.amountTap('), 'coach mode tools hidden while viewing someone else');
  ...
  const shell = fn('renderApp')(App.state);
  assert(!shell.includes('coach-banner'), 'coach banner hidden on the week page while viewing someone else');
